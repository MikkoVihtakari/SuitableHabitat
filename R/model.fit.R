...
out <- list(polygon = pol.dat, hexagon = hex.dat, parameters = mod$parameters)
...
# 
# 
# reg.plots <- lapply(regdf, function(k) {
#   
...
#   ## Region borders
#   
#   reg.pol <- regions[regions@data$region == reg,]
#   reg.pol.dt <- suppressMessages(suppressWarnings(broom::tidy(reg.pol)))
#   
#   coord.limits <- PlotSvalbard::auto_limits("panarctic", limits = c("long", "lat"), data = reg.pol.dt)
#   
#   ## Plot
#   
#   basemap("panarctic", limits = coord.limits, base_size = 8) + 
...
#     geom_polygon(data = reg.pol.dt, aes(x = long, y = lat, group = group), fill = NA, color = "black") +
#     ggtitle(reg) + 
#     add_land()
#   
# })
